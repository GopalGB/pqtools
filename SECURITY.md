...
`pqtools` reads and rewrites Power Query M source files. The core makes no
...
`pq --write` snapshots the file (identity, size, timestamp, content), applies the
...
cooperating `pq` processes, not a correctness guarantee: the lock file is removed
