...
## Round 57 - `6cb64f6..afc9a92`, verdict FIX-FIRST, all eight taken

Every finding is in the script written last round to end this class. That is
the expected shape - a new mechanism has new defects - and two of them are
sharp enough to be worth reading twice.

### 1 HIGH - the recorded provenance could be blank and still report success

`echo "#   pushed tip : $(git rev-parse --short '@{upstream}')"` - a command
substitution inlined into `echo` cannot fail under `set -e`, because the exit
status belongs to `echo`. Verified in a detached worktree, which is where the
review wrapper runs: `git rev-parse --short '@{upstream}'` prints *fatal: HEAD
does not point to a branch* and **the line survives with exit 0**, writing a
blank provenance field and a false ` (diverged)` marker while announcing
`wrote ... digest stable`.

Every measurement is now its own statement, and the tree is pinned by
`HEAD^{tree}`, which always resolves. This is the same shape as SC2155, which
this script had already taken - and I still wrote the bug one line lower.

### 2 HIGH - the falsifiability guard was itself unfalsifiable

The guard required `short test summary info` to be present, as proof `-rs`
applied. But pytest's `-r` default is `fE` (verified: `_REPORTCHARS_DEFAULT ==
'fE'`), so that block prints on **any** run with a failure. Measured: a failing
run with no `-rs` at all prints it. On a red run the guard would green and the
argv claim would be back to unfalsifiable.

**The reviewer's proposed fix was also wrong, and reproducing it is what
caught that.** They suggested asserting the SKIPPED *line* count equals the
summary's skip number. Measured: 37 lines against 41 skips - pytest groups
skips by (file, line, reason) and prints `SKIPPED [n]`. The exact invariant is
the **sum of the bracketed counts**, which is 41.

| # | Condition | Observable | Exit |
|---|---|---|---|
| SS1c | a file that skips, run without `-rs` | `SKIPPED lines: 0  accounting for: 0` / `summary says skipped: 6` | 4, no log |
| SS2 | same file with `-rs` | `skips: 6 SKIPPED lines accounting for 6, summary says 6` | 0, written |
| SS3 | a red suite | `FLOOR EXIT: 1` in the log **and** exit 1 from the script | 1 |

**A gap the review did not name, found by its own control.** SS1's first run -
red, no `-rs`, no skips - was *written*, because `0 == 0` satisfied the
accounting check trivially. A floor run with zero skips is not a floor run:
the extras-dependent tests are exactly what absent extras make unrunnable. The
script now refuses that outright (SS1b, exit 6).

### 3-8 - the rest, all measured

| finding | what was wrong | fix |
|---|---|---|
| MEDIUM | a red suite wrote the log and exited **0**; the only signal was inside the artifact | `exit "${CODE}"` (SS3) |
| MEDIUM | `${ARGS[*]}` joins with spaces, and the rootdir contains one - so the recorded line was **byte-identical** to the one-giant-argument string it exists to disprove (measured: the two forms compare equal) | one `%q`-quoted word per line |
| MEDIUM | `.venv/bin/python` hard-coded; no `.venv` exists in a review worktree, so the script died bare-handed after carefully validating the other interpreter | optional `$3`, both interpreters validated with a named error |
| MEDIUM | the digest excluded `scripts/`, so the artifact claimed "produced by this script" while pinning no version of it | `scripts` added to scope |
| LOW | `xargs shasum` splits on spaces, and `-co` had just pulled untracked paths into scope | `-z` / `sort -z` / `xargs -0` |
| LOW | `local HEAD at run: 21ed1ca` was a dangling autocommit - the unresolvable-sha defect had *moved down one line*, not gone | record `HEAD^{tree}` |

### The artifact is now checked, not merely self-consistent

The last finding was that nothing verifies the committed log against the tree.
The reviewer suggested a test. **A test cannot work here, and the reason is
worth recording:** the suite runs *inside* the floor run, where the log on disk
is still the previous one. Such a test could never go green - the log can only
be regenerated by a run that the test itself would fail.

So it is **gate step 9** instead, which is where "is this evidence current"
belongs anyway: a stale artifact should block shipping, not block editing.
